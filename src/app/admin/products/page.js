        ...
        <div>
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Gérez votre catalogue, ajoutez ou modifiez des fiches techniques.</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-12 text-center text-slate-500">Aucun produit trouvé. Commencez par en ajouter un.</td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={product._id} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="w-10 h-10 rounded bg-slate-200 flex-shrink-0 relative overflow-hidden">
                                                    {product.images?.[0] ? <img src={product.images[0]} alt="" className="w-full h-full object-cover" /> : null}
                                                </div>
                                                <span className="font-medium text-slate-900">{product.designation}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-sm text-slate-500">{product.caracteristiques?.rendement || '-'}</td>
                                        <td className="px-6 py-4 text-sm text-slate-500 font-medium">{product.point_fidelite} pts</td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <Link href={`/admin/products/edit/${product._id}`} className="p-2 text-slate-400 hover:text-primary transition-colors hover:bg-blue-50 rounded-lg">
                                                    <Edit size={16} />
                                                </Link>
                                                <button
