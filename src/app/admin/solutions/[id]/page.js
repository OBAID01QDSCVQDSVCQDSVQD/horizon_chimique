...
export default function EditSolutionPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch Solution Data
                const resSol = await fetch(`/api/solutions/${id}`);
                const dataSol = await resSol.json();
                if (!dataSol.success) throw new Error('Solution introuvable');
                ...
                // Populate Form
                const sol = dataSol.data;
                setFormData({
                    title: sol.title,
                    description: sol.description,
                    features: Array.isArray(sol.features) ? sol.features.join('\n') : sol.features, // Convert array to multiline string
                    icon: sol.icon || 'Layers',
                    color: sol.color || 'bg-blue-500',
                    relatedProducts: sol.relatedProducts || []
                });
        ...
        <div className="max-w-5xl mx-auto pb-20">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30">
                    <h1 className="text-xl font-bold text-slate-800 dark:text-white">Modifier la Solution</h1>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-8">
                    ...
                    <div className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <input name="title" required value={formData.title} onChange={handleChange} className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary/20 outline-none bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
                        </div>
                    ...
                    <button type="submit" disabled={submitting} className="w-full bg-primary text-white font-bold py-3 rounded-xl hover:bg-primary-dark transition-colors flex items-center justify-center gap-2 shadow-lg hover:shadow-primary/20 mt-4">
                        {submitting ? <Loader2 className="animate-spin" /> : <Save />} Enregistrer les Modifications
                    </button>

                    {/* Custom Styles for Quill in Dark Mode - Duplicated from new/page.js */}
                    <style jsx global>{`
                        .ql-toolbar {
